#!/usr/bin/env bash
# Resolve project CLI flags to environment variables, then exec the service.
set -euo pipefail
...
if [[ -z "$bin" ]]; then
  if [[ -x "$root/vendor/flags-2-env/build/flags2env" ]]; then
    ...
    echo "flags2env: build the pinned submodule with 'make -B -C vendor/flags-2-env all' or set FLAGS2ENV_BIN" >&2
    exit 127
  fi
fi
