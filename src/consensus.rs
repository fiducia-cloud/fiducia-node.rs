...
#[path = "raft_callback.rs"]
mod raft_callback;
pub use self::raft_callback::RequestStamp;
use self::raft_callback::{
...
struct LeaderState {
    ...
    /// Dispatch instant of the latest acknowledged request in this term —
    /// never reply arrival, which could renew already expired evidence. Drives CheckQuorum / the leader
...
impl ShardActor {
    ...
    fn new(
        ...
        let mut actor = ShardActor {
            ...
            storage_fault: None,
            votes: HashSet::new(),
            campaign_started_at: Instant::now(),
    ...
    fn become_leader(&mut self) {
        ...
        // Votes establish contact no earlier than campaign dispatch. Never
    ...
    fn send_append_to(&mut self, peer: &str) {
        ...
        let Some(sequence) = ls.request_sequence.checked_add(1) else {
            tracing::warn!(
                shard = self.shard_id,
                "raft callback sequence exhausted; stepping down"
            );
    ...
    /// Validate before touching any leader bookkeeping, even for timeouts.
    fn accept_replication_callback(
        ...
        let active = if self.role == Role::Leader {
            self.leader
                .as_ref()
                .and_then(|leader| leader.pending_requests.get(from))
                .copied()
        } else {
            None
        };
        match classify_callback(
            ...
            CallbackAdmission::HigherTerm(term) => {
            ...
            CallbackAdmission::Current => {
                if let Some(leader) = self.leader.as_mut() {
                    leader.pending_requests.remove(from);
                }
                true
            }
        }
    ...
    fn handle_append_reply(
        ...
        let mut more = false;
        if let Some(ls) = self.leader.as_mut() {
            // Bound evidence by dispatch, not arrival: transport and inbox
            ...
            if successful {
                ...
                let acknowledged_next = ls
                    ...
                    .saturating_add(1);
                ls.next_index
                    .insert(from.clone(), backoff.max(acknowledged_next));
    ...
    fn handle_snapshot_reply(
        ...
        if resp.success {
            if let Some(leader) = self.leader.as_mut() {
                ...
                let Some((matched, next)) = acknowledged_prefix(known, up_to, resp.match_index)
                else {
                    return;
                };
