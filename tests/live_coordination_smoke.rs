...
async fn live_higher_level_primitives_smoke() -> TestResult {
    ...
    let r = call(
        ...
        post.clone(),
        "/v1/counters/add",
        Some(json!({"key": ckey, "delta": 100, "prev_revision": 0})),
    )
    .await?;
    ...
    call(
        ...
        post.clone(),
        "/v1/barriers/create",
        Some(json!({"name": bname, "policy": {"kind": "quorum", "required": 2}, "expected": 3})),
    )
    .await?;
    let r = call(
        &client,
        ...
        post.clone(),
        "/v1/barriers/arrive",
        Some(json!({"name": bname, "participant": "a"})),
    )
    .await?;
    ...
    let r = call(
        ...
        post.clone(),
        "/v1/barriers/arrive",
        Some(json!({"name": bname, "participant": "b"})),
    )
    .await?;
    assert_eq!(
        r["barrier"]["status"], "satisfied",
        "2nd distinct arrival resolves"
    );
    ...
    call(
        ...
        post.clone(),
        "/v1/tasks/create",
        Some(json!({"name": tname, "task_type": "impl"})),
    )
    .await?;
    let r = call(
        &client,
        ...
        post.clone(),
        "/v1/tasks/claim",
        Some(json!({"name": tname, "worker": "agent-a", "ttl_ms": 60000})),
    )
    .await?;
    ...
    let r = call(
        ...
        post.clone(),
        "/v1/tasks/claim",
        Some(json!({"name": tname, "worker": "agent-b"})),
    )
    .await?;
    ...
    let r = call(
        ...
        "/v1/tasks/progress",
        Some(
            json!({"name": tname, "worker": "agent-a", "fencing_token": tok + 999, "percent": 50}),
        ),
    )
    .await?;
    ...
    let r = call(
        ...
        "/v1/tasks/complete",
        Some(
            json!({"name": tname, "worker": "agent-a", "fencing_token": tok, "result": {"pr": 1}}),
        ),
    )
    .await?;
    ...
    call(
        ...
        post.clone(),
        "/v1/effects/approve",
        Some(json!({"name": ename, "principal": "finance-a"})),
    )
    .await?;
    call(
        &client,
    ...
    call(
        ...
        post.clone(),
        "/v1/effects/approve",
        Some(json!({"name": ename, "principal": "finance-b"})),
    )
    .await?;
    let r = call(
        &client,
        ...
        post.clone(),
        "/v1/effects/commit",
        Some(json!({"name": ename, "result": {"c": "ok1"}})),
    )
    .await?;
    ...
    let r = call(
        ...
        post.clone(),
        "/v1/effects/commit",
        Some(json!({"name": ename, "result": {"c": "ok2"}})),
    )
    .await?;
    ...
    let r = call(
        ...
        &base,
        get.clone(),
        &format!("/v1/effects?name={ename}"),
        None,
    )
    .await?;
    assert_eq!(
        r["effect"]["result"]["c"], "ok1",
    ...
    call(
        ...
        post.clone(),
        "/v1/tasks/create",
        Some(json!({"name": hres, "task_type": "research"})),
    )
    .await?;
    let r = call(
        &client,
        ...
        post.clone(),
        "/v1/tasks/claim",
        Some(json!({"name": hres, "worker": "research-agent"})),
    )
    .await?;
    ...
    let r = call(
        ...
        post.clone(),
        "/v1/handoffs/accept",
        Some(json!({"name": hname, "to": "legal-agent"})),
    )
    .await?;
    assert!(
        r["to_token"].as_u64().unwrap() > ftok,
        "accept mints strictly higher token"
    );
    ...
    call(
        ...
        post.clone(),
        "/v1/decisions/vote",
        Some(json!({"name": dname, "voter": "a", "option": "approve", "weight": 1})),
    )
    .await?;
    call(
        &client,
        ...
        post.clone(),
        "/v1/decisions/vote",
        Some(json!({"name": dname, "voter": "b", "option": "reject", "weight": 1})),
    )
    .await?;
    let r = call(
        &client,
        &base,
        get.clone(),
        &format!("/v1/decisions?name={dname}"),
        None,
    )
    .await?;
    ...
    call(
        ...
        post.clone(),
        "/v1/decisions/vote",
        Some(json!({"name": dname, "voter": "c", "option": "approve", "weight": 5})),
    )
    .await?;
    let r = call(
        &client,
    ...
    .await?;
    assert_eq!(
        r["decision"]["winner"], "approve",
        "resolves to heaviest option"
    );
    let r = call(
        &client,
        &base,
        post.clone(),
        "/v1/decisions/vote",
        Some(json!({"name": dname, "voter": "d", "option": "maybe"})),
    )
    .await?;
    ...
    call(
        ...
        post.clone(),
        "/v1/budgets/set",
        Some(json!({"name": gname, "limit": {"usd_micros": 1000000}})),
    )
    .await?;
    ...
    call(
        ...
        post.clone(),
        "/v1/budgets/commit",
        Some(json!({"name": gname, "reservation_id": "a", "actual": {"usd_micros": 200000}})),
    )
    .await?;
    let r = call(
        &client,
        &base,
        get.clone(),
        &format!("/v1/budgets?name={gname}"),
        None,
    )
    .await?;
    assert_eq!(
        r["budget"]["available"]["usd_micros"], 800000,
        "commit frees the difference"
    );
    ...
    call(
        ...
        post.clone(),
        "/v1/claims/support",
        Some(json!({"name": clname, "agent": "audit"})),
    )
    .await?;
    let r = call(
        &client,
        ...
        post.clone(),
        "/v1/claims/contest",
        Some(json!({"name": clname, "agent": "fraud", "reason": "chargeback"})),
    )
    .await?;
    ...
    call(
        ...
        post.clone(),
        "/v1/claims/resolve",
        Some(json!({"name": clname, "accepted": true})),
    )
    .await?;
    let r = call(
        &client,
        ...
        post.clone(),
        "/v1/claims/support",
        Some(json!({"name": clname, "agent": "late"})),
    )
    .await?;
